...
export function ContactForm({ className = '' }: ContactFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mailtoLink = `mailto:hello@flowency.co.uk?subject=${encodeURIComponent(subject)}&body=${body}`
    ...
    <form onSubmit={handleSubmit} className={`space-y-6 ${className}`}>
      ...
      {submitStatus === 'error' && (
        <p className="text-center text-sm text-destructive bg-destructive/10 border border-destructive/30 rounded-xl py-3">
          Something went wrong. Please try again or email us directly at hello@flowency.co.uk
        </p>
      )}
    </form>
  )
